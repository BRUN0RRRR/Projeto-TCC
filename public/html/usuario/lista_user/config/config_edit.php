...
$alt_senha = isset($_POST['alterarSenhaCheck']) ? $_POST['alterarSenhaCheck'] : 0;
echo "Valor recebido: " . $alt_senha;
...
$stmt->bindParam(":cod_usuario", $login, PDO::PARAM_STR);
$stmt->execute();
    ...
    if ($result['nome'] === $nome) {
        if ($result['email'] === $email) {
            if (empty($senha) && empty($senhaCon)) {
                $stmt = $pdo->prepare("UPDATE cadastro_user SET nome = :nome, email = :email, setor = :setor, perfil = :perfil, status_user = :status_user, alt_senha = :alt_senha  WHERE cod_usuario = :cod_usuario");
            ...
            $stmt = $pdo->prepare("UPDATE cadastro_user SET nome = :nome, email = :email, senha = :senha, setor = :setor, perfil = :perfil, status_user = :status_user, alt_senha = :alt_senha WHERE cod_usuario = :cod_usuario");
