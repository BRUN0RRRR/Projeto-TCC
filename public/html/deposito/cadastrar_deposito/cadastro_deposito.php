...
<head>
    ...
    <title> Infostock - Gestão Depositos</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.2/font/bootstrap-icons.min.css">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <link rel="stylesheet" href="css/pagina_deposito.css">
...
<body>
    ...
    <div class="container">
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <div class="mb-3">
                    <span><i class="fas fa-tag me-1"></i> Total de Depositos: <?= $total_custo['total']; ?></span>
                </div>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php foreach ($depositos_select as $depositos): ?>
                                <tr data-produto-id="<?= $depositos['id_deposito']; ?>">
                                    <td><?= $depositos['id_deposito']; ?></td>
                                    <td><?= $depositos['nome']; ?></td>
                                    <td><?= $depositos['descricao']; ?></td>
                                    <td>
                                        <span class="status-badge status-<?= strtolower($depositos['status']) ?>">
                                            <?= htmlspecialchars($depositos['status']) ?>
                                        </span>
                                    </td>
                                    <td>
                                        <button class="btn btn-sm btn-warning btn-edit" data-bs-toggle="modal"
                                            data-bs-target="#editCategoryModal" data-id="<?= $depositos['id_deposito']; ?>"
                                            data-nome="<?= htmlspecialchars($depositos['nome'], ENT_QUOTES); ?>"
                                            data-codigo="<?= htmlspecialchars($depositos['id_deposito'], ENT_QUOTES); ?>"
                                            data-descricao="<?= htmlspecialchars($depositos['descricao'], ENT_QUOTES); ?>"
                                            data-status="<?= $depositos['status']; ?>">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Novo Depositos</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <form id="createCategoryForm" action="config/config_criar_deposito.php" method="POST">
                        <!-- <input type="hidden" id="edit_id" name="id_categoria">-->
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="code" class="form-label">Código</label>
                                <input type="text" class="form-control" id="id_deposito" name="id_deposito" required>
                            </div>
                            <div class="mb-3">
        ...
        <div class="modal fade" id="editCategoryModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Editar Depositos</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>

                    </div>
                    <form id="editCategoryForm" action="config/config_edite_deposito.php" method="POST">
                    <input type="hidden" name="id_original"  ?>
                    <input type="hidden" id="edit_id" name="id_original" value="<?php echo $deposito['id_deposito'] ?>">
                   

                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="edit_code" class="form-label">Código</label>
                                <input type="text" class="form-control" id="edit_code" name="id_deposito" required>
                            </div>
                            <div class="mb-3">
